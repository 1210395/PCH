        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-6 sm:mb-8">
            ...
            <div>
                ...
                <p class="text-white/90 text-sm leading-relaxed">
                    ...
                    @else
                        {{ $footerSettings['description'] ?? __('A digital hub and marketplace supporting designers, MSMEs, and creative industries in Palestine. Connecting talent with opportunities.') }}
                    @endif
                </p>
                @php
            ...
            <!-- Quick Links -->
            <div>
                <h3 class="mb-4">{{ __('Quick Links') }}</h3>
                <ul class="space-y-3">
                    ...
                    @endphp
                    <li>
                        <a href="{{ $href }}" class="text-white/90 hover:text-white transition-colors text-sm" @if($isExternal) target="_blank" rel="noopener noreferrer" @endif>
                            {{ $linkTitle }}
                        </a>
                    </li>
                    @endforeach
            ...
            <!-- Contact Info -->
            <div>
                <h3 class="mb-4">{{ __('Contact Us') }}</h3>
                <ul class="space-y-3">
                    @if(!empty($footerSettings['contact']['address']))
                    <li class="flex items-start gap-2">
                        ...
                        <span class="text-white/90 text-sm">
                            {!! nl2br(e($footerSettings['contact']['address'])) !!}
                        </span>
                    </li>
                    @endif
                    @if(!empty($footerSettings['contact']['email']))
                    <li class="flex items-center gap-2">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                        </svg>
                        <a href="mailto:{{ $footerSettings['contact']['email'] }}" class="text-white/90 hover:text-white transition-colors text-sm">
                            {{ $footerSettings['contact']['email'] }}
                        </a>
                    </li>
                    @endif
                    @if(!empty($footerSettings['contact']['phone']))
                    <li class="flex items-center gap-2">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                        </svg>
                        <a href="tel:{{ preg_replace('/\s+/', '', $footerSettings['contact']['phone']) }}" class="text-white/90 hover:text-white transition-colors text-sm" dir="ltr">
        ...
        <div class="pt-8 border-t border-white/20">
            <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                <p class="text-white/90 text-sm">
                    @if(app()->getLocale() === 'ar' && !empty($footerSettings['copyright_ar']))
                        {{ $footerSettings['copyright_ar'] }}
                    @else
                        {{ $footerSettings['copyright'] ?? '© ' . date('Y') . ' Palestine Creative Hub. All rights reserved.' }}
                    @endif
                </p>
                @if(!empty($footerSettings['bottom_links']))
                <div class="flex items-center gap-6">
                    @foreach($footerSettings['bottom_links'] as $link)
                    @php
                    ...
                    @endphp
                    <a href="{{ $bottomHref }}" class="text-white/90 hover:text-white transition-colors text-sm" @if($isExternal) target="_blank" rel="noopener noreferrer" @endif>
                        {{ $bottomLinkTitle }}
                    </a>
                    @endforeach
                </div>
