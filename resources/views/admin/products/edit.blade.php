...
@section('breadcrumb')
    <a href="{{ route('admin.products.index', ['locale' => app()->getLocale()]) }}" class="text-blue-600 hover:underline">{{ __('Products') }}</a>
    <i class="fas fa-chevron-right text-gray-400 text-xs mx-2"></i>
    <span class="text-gray-700">Edit: {{ Str::limit($product->title ?? $product->name, 30) }}</span>
@endsection

@section('content')
<div x-data="productForm()" class="max-w-4xl">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">{{ __('Edit Product') }}</h1>
            <p class="text-gray-500">{{ __('Update product details') }}</p>
        </div>
        <div class="flex items-center gap-2">
            @if($product->designer)
                <a href="{{ url(app()->getLocale() . '/designer/' . $product->designer->id) }}" target="_blank" class="px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-lg">
                    <i class="fas fa-external-link-alt mr-2"></i>{{ __('View Designer Profile') }}
                </a>
            @endif
            <a href="{{ route('admin.products.index', ['locale' => app()->getLocale()]) }}" class="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">
    ...
    <form @submit.prevent="submitForm()" class="space-y-6">
                ...
                <div>
                    ...
                    <div x-data="searchableCategory()" class="relative">
                        <input
                            ...
                            :placeholder="selectedValue || 'Select product category'"
        ...
        <!-- Product Images -->
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ __('Product Images (Max 6)') }}</h3>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <label for="featured" class="text-sm font-medium text-gray-700">{{ __('Featured Product') }}</label>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="flex items-center gap-4">
                @if($product->designer && $product->designer->avatar)
                    <img src="{{ asset('storage/' . $product->designer->avatar) }}" class="w-12 h-12 rounded-full object-cover">
                @else
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-green-500 flex items-center justify-center text-white font-bold">
                        {{ substr($product->designer->name ?? 'D', 0, 1) }}
                    </div>
                @endif
                <div>
                    <p class="font-medium text-gray-800">{{ $product->designer->name ?? __('Unknown') }}</p>
                    <p class="text-sm text-gray-500">{{ $product->designer->email ?? '' }}</p>
                </div>
                @if($product->designer)
                    <a href="{{ route('admin.designers.show', ['locale' => app()->getLocale(), 'id' => $product->designer->id]) }}" class="ml-auto px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-lg text-sm">
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.products.index', ['locale' => app()->getLocale()]) }}" class="px-6 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">{{ __('Cancel') }}</a>
    ...
    <div x-show="showRejectModal" x-cloak @click.self="showRejectModal = false" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div @click.stop class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 class="text-lg font-semibold mb-4">{{ __('Reject Product') }}</h3>
...
function searchableCategory() {
    const initialValue = @json($product->category ?? '');
    ...
    return {
        ...
        selectOption(option) {
            ...
            this.isOpen = false;
            // Update parent form
            const form = Alpine.$data(this.$root.closest('[x-data*="productForm"]'));
...
function productForm() {
    ...
    return {
        ...
        handleImageUpload(event, maxImages) {
            ...
            const currentCount = {{ $product->images ? $product->images->count() : 0 }};
        ...
        async deleteExistingImage(imageId) {
            ...
            try {
                await adminFetch(`{{ url('') }}/{{ app()->getLocale() }}/admin/products/{{ $product->id }}/images/${imageId}`, {
                    method: 'DELETE'
                });
        ...
        async submitForm() {
            ...
            try {
                ...
                const response = await fetch(`{{ url('') }}/{{ app()->getLocale() }}/admin/products/{{ $product->id }}`, {
                    method: 'POST',
                    body: formData,
                    headers: {
                ...
                showToast('{{ __("Product updated successfully") }}', 'success');
                setTimeout(() => window.location.href = `{{ url('') }}/{{ app()->getLocale() }}/admin/products`, 1000);
            } catch (e) {
        ...
        async quickApprove() {
            try {
                await adminFetch(`{{ url('') }}/{{ app()->getLocale() }}/admin/products/{{ $product->id }}/approve`, { method: 'POST' });
                showToast('{{ __("Product approved") }}', 'success');
        ...
        async submitReject() {
            try {
                await adminFetch(`{{ url('') }}/{{ app()->getLocale() }}/admin/products/{{ $product->id }}/reject`, {
                    method: 'POST',
                    body: JSON.stringify({ reason: this.rejectReason })
                });
                showToast('{{ __("Product rejected") }}', 'success');
