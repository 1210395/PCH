...
<div x-data="fablabForm()" class="max-w-4xl">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">{{ __('Add New FabLab') }}</h1>
            <p class="text-gray-500">{{ __('Create a new fabrication laboratory listing') }}</p>
        </div>
    ...
    <form @submit.prevent="submitForm()" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div>
                ...
                <template x-if="imagePreview">
                    <img :src="imagePreview" class="mt-2 w-48 h-32 object-cover rounded-lg">
                </template>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" :disabled="submitting" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
                <span x-show="!submitting">{{ __('Create FabLab') }}</span>
...
function fablabForm() {
    return {
        form: {
            name: '', type: '', city: '', location: '', short_description: '', description: '',
            phone: '', email: '', website: '', opening_hours: '',
            rating: 0, reviews_count: 0, members: 0, verified: false
        },
        equipmentInput: '', servicesInput: '', featuresInput: '',
        ...
        async submitForm() {
            ...
            try {
                ...
                const response = await fetch(`{{ url('') }}/{{ app()->getLocale() }}/admin/fablabs`, {
                    method: 'POST',
                ...
                showToast('{{ __("FabLab created successfully") }}', 'success');
