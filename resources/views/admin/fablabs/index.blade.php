        ...
        <form method="GET" action="{{ route('admin.fablabs.index', ['locale' => app()->getLocale()]) }}" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[250px]">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search by name, location, or city...') }}" class="w-full pl-10 rtl:pr-10 rtl:pl-4 pr-4 py-2 border rounded-lg">
                </div>
            </div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-3">
                                @if($fablab->image)
                                    <img src="{{ asset('storage/' . $fablab->image) }}" alt="" class="w-12 h-12 rounded-lg object-cover">
                                @else
