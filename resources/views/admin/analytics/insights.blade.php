...
@endphp
<div class="space-y-6">
    @include('admin.analytics._header', ['exportRoute' => 'admin.analytics.insights.export'])

    <div x-data="{
        severity: 'all',
        source:   'all',
        items: {{ Js::from($insights) }},
        get filtered() {
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex flex-wrap items-center gap-4">
            ...
            <div class="flex flex-wrap gap-2 flex-1">
                <button @click="severity = 'all'"
                    ...
                    <i class="fas fa-list text-xs"></i> {{ __('All') }}
                    <span class="bg-gray-300 text-gray-700 text-xs font-bold px-1.5 py-0.5 rounded-full">{{ count($insights) }}</span>
                </button>
                @if($criticalCount)
                <button @click="severity = 'critical'"
                    ...
                    <i class="fas fa-exclamation-circle text-xs"></i> {{ __('Critical') }}
                    <span class="bg-red-200 text-red-700 text-xs font-bold px-1.5 py-0.5 rounded-full">{{ $criticalCount }}</span>
                </button>
                @endif
                @if($warningCount)
                <button @click="severity = 'warning'"
                    ...
                    <i class="fas fa-exclamation-triangle text-xs"></i> {{ __('Warning') }}
                    <span class="bg-amber-200 text-amber-700 text-xs font-bold px-1.5 py-0.5 rounded-full">{{ $warningCount }}</span>
                </button>
                @endif
                @if($infoCount)
                <button @click="severity = 'info'"
                    ...
                    <i class="fas fa-info-circle text-xs"></i> {{ __('Info') }}
                    <span class="bg-blue-200 text-blue-700 text-xs font-bold px-1.5 py-0.5 rounded-full">{{ $infoCount }}</span>
                </button>
                @endif
            </div>
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <span class="text-xs text-gray-500 font-medium">{{ __('Source:') }}</span>
        ...
        <div class="space-y-3">
                    ...
                    <div class="flex flex-wrap items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap gap-1">
                                <span class="text-xs text-gray-400">{{ __('Data from:') }}</span>
            ...
            <div x-show="filtered.length === 0" class="bg-white rounded-xl shadow-sm p-16 text-center">
                ...
                <p class="text-gray-600 font-medium">{{ __('No insights match the selected filters.') }}</p>
                <p class="text-gray-400 text-sm mt-1">{{ __('Try changing the severity or source filter, or adjust the date range.') }}</p>
            </div>

            {{-- All-clear state when no insights at all --}}
            @if(count($insights) === 0)
            <div class="bg-white rounded-xl shadow-sm p-16 text-center">
                <i class="fas fa-check-double text-4xl text-green-400 mb-3"></i>
                <p class="text-green-700 font-semibold text-lg">{{ __('Everything looks healthy!') }}</p>
                <p class="text-gray-400 text-sm mt-1">{{ __('No issues detected across all analytics sources for the selected period.') }}</p>
            </div>
            @endif
        </div>
