...
@php
    $imageUrl = $fabLab->cover_image ? url('media/' . $fabLab->cover_image) : ($fabLab->image ? url('media/' . $fabLab->image) : null);
   ...
   class="group bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col h-full">
    ...
    <div class="relative aspect-[4/3] flex-shrink-0 overflow-hidden bg-gray-100">
        @if($imageUrl)
            ...
            >
        @else
            <div class="absolute inset-0 w-full h-full bg-gradient-to-br from-blue-500 to-green-400 flex items-center justify-center">
