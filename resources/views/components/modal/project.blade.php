...
<div x-show="projectModal"
    ...
    <div @click.stop
         ...
         class="bg-white rounded-t-2xl sm:rounded-2xl sm:rounded-3xl shadow-2xl max-w-2xl w-full max-h-[100dvh] sm:max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div class="relative flex items-start justify-between">
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <h2 class="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight" x-text="projectForm.id ? '{{ __('Edit Project') }}' : '{{ __('New Project') }}'"></h2>
                    </div>
                    <p class="text-white/90 text-xs sm:text-sm ml-12 sm:ml-15" x-text="projectForm.id ? '{{ __('Update your project details below') }}' : '{{ __('Add a new project to your portfolio') }}'"></p>
                </div>
            ...
            <div class="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
                ...
                <div class="group">
                    ...
                    <input type="text"
                           ...
                           placeholder="{{ __('e.g., Modern Villa Design') }}"
                ...
                <div class="group">
                    ...
                    <select x-model="projectForm.category"
                            @change="if (projectForm.category !== 'Other') { projectForm.customCategory = '' }"
                        ...
                        @foreach(\App\Helpers\DropdownHelper::projectCategories() as $category)
                            <option value="{{ $category }}">{{ $category }}</option>
                        @endforeach
                    </select>
                    <input x-show="projectForm.category === 'Other'"
                           x-transition:enter="transition ease-out duration-300"
                           x-transition:enter-start="opacity-0 transform -translate-y-2"
                           x-transition:enter-end="opacity-100 transform translate-y-0"
                           type="text"
                           x-model="projectForm.customCategory"
                           placeholder="{{ __('Specify your category') }}"
                ...
                <div class="group">
                    ...
                    <textarea x-model="projectForm.description"
                              placeholder="{{ __('Describe your project\'s goals, challenges, and solutions...') }}"
                              rows="4"
                              maxlength="500"
                    ...
                    <p class="mt-2 text-xs text-gray-500 flex items-center gap-1.5">
                        ...
                        <span x-text="(projectForm.description ? projectForm.description.length : 0) + '/500'"></span> {{ __('characters') }}
                    </p>
                </div>
                ...
                <div class="group">
                    ...
                    <select x-model="projectForm.role"
                            @change="if (projectForm.role !== 'Other') { projectForm.customRole = '' }"
                            class="w-full px-5 py-3.5 border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all duration-200 outline-none appearance-none bg-white group-hover:border-gray-300 cursor-pointer text-gray-900 font-medium"
                            style="background-image: url(&quot;data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M6 8l4 4 4-4'/%3e%3c/svg%3e&quot;); background-position: right 1rem center; background-repeat: no-repeat; background-size: 1.5em 1.5em; padding-right: 3rem;">
                        <option value="">{{ __('Select your role') }}</option>
                        @foreach(\App\Helpers\DropdownHelper::projectRoles() as $role)
                            <option value="{{ $role }}">{{ $role }}</option>
                        @endforeach
                    </select>
                    <input x-show="projectForm.role === 'Other'"
                           x-transition:enter="transition ease-out duration-300"
                           x-transition:enter-start="opacity-0 transform -translate-y-2"
                           x-transition:enter-end="opacity-100 transform translate-y-0"
                           type="text"
                ...
                {{-- Project Images --}}
                <div class="group">
                    <label class="block text-sm font-bold text-gray-800 mb-2.5">{{ __('Project Images') }}</label>
                    <div class="space-y-4">
                        <div class="relative">
                            <input type="file"
                                   accept="image/*"
                        ...
                        {{-- Upload Progress --}}
                        <div x-show="projectForm.uploading"
                             x-transition
                        ...
                        {{-- Image Previews --}}
                        <div x-show="projectForm.images && projectForm.images.length > 0"
                             x-transition
        ...
        <div class="flex-shrink-0 p-4 sm:p-6 border-t border-gray-100 bg-gradient-to-r from-gray-50 to-gray-100/50 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-end safe-area-bottom">
            ...
            <button @click="saveProject()"
                ...
                <svg x-show="!projectSubmitting" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
