            ...
            <div x-show="currentStep === 5" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform translate-x-4" x-transition:enter-end="opacity-100 transform translate-x-0">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 md:p-8 max-w-4xl mx-auto overflow-visible">
                    <h2 class="text-xl font-bold text-gray-900 mb-2">{{ __('Sample Projects') }}</h2>
                    <p class="text-gray-600 mb-6">{{ __("Showcase projects you've worked on (optional but recommended)") }}</p>

                    <!-- Error Messages -->
                    <div x-show="errors.projects" class="mb-4 bg-red-50 border-l-4 border-red-500 rounded-lg p-4 animate-fade-in">
                        <div class="flex items-start">
                            ...
                            <p class="text-sm text-red-700" x-text="errors.projects"></p>
                        </div>
                    </div>

                    <div class="space-y-6 overflow-visible">
                        <template x-for="(project, index) in formData.projects" :key="project.id">
                            <div class="bg-gray-50 rounded-lg p-4 sm:p-5 md:p-6 border border-gray-200 overflow-visible">
                                <div class="flex items-start justify-between mb-4">
                                    <h3 class="font-semibold text-gray-900">{{ __('Project') }} <span x-text="index + 1"></span></h3>
                                    <button
                                        type="button"
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">
                                            {{ __('Project Images (Max 6 images, 5MB each)') }}
                                        </label>

                                        <!-- Image Previews Grid -->
                                        <div x-show="project.images && project.images.length > 0" class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                                            <template x-for="(image, imgIndex) in project.images" :key="image.id">
                                                <div class="relative group">
                                                    <div class="relative w-full h-32 rounded-lg overflow-hidden bg-gray-200 border-2 border-gray-300">
                                                        <img :src="image.preview" :alt="'Project image ' + (imgIndex + 1)" class="w-full h-full object-cover">
                                                    </div>
                                                    <button
                                                        type="button"
                                                        @click="removeProjectImage(project.id, image.id)"
                                                    ...
                                                    <span class="absolute bottom-1 right-1 bg-black bg-opacity-60 text-white text-xs px-2 py-1 rounded">
                                                        <span x-text="imgIndex + 1"></span>/<span x-text="project.images.length"></span>
                                                    </span>
                                                </div>
                                            </template>
                                        </div>
                                        ...
                                        <div x-show="!project.images || project.images.length < 6">
                                            <label
                                                :for="'project-images-' + project.id"
                                                ...
                                                @drop.prevent="handleProjectImageChange(project.id, $event); $el.classList.remove('border-blue-500', 'bg-blue-50')"
                                                :class="project.isUploadingImage ? 'pointer-events-none opacity-60' : ''"
                                            >
                                                <div class="flex flex-col items-center justify-center text-center">
                                                    <div x-show="project.isUploadingImage" class="flex flex-col items-center">
                                                    ...
                                                    <div x-show="!project.isUploadingImage">
                                                        ...
                                                        <p class="text-xs text-gray-500 mt-1">
                                                            <span x-text="project.images ? `${project.images.length}/6 images ` : '0/6 images '"></span>
                                                ...
                                                <input
                                                    ...
                                                    accept="image/*"
                                                    multiple
                                                    @change="handleProjectImageChange(project.id, $event)"
                                                    class="hidden"
                                                >
                                            </label>
                                        </div>
                                        <p x-show="project.images && project.images.length >= 6" class="text-sm text-green-600 mt-1">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Project Title') }} *</label>
                                            <input
                                                type="text"
                                        ...
                                        <div>
                                            ...
                                            <div x-data="searchableSelectProjectCategory(project.id)" class="relative">
                                                <input type="hidden" :name="'projects[' + index + '][category]'" :value="selectedValue" x-model="selectedValue">
                                                <div class="relative">
                                                    <input
                                                        type="text"
                                                        x-model="searchQuery"
                                            ...
                                            <input
                                                x-show="project.category === 'Other'"
                                                type="text"
                                                :name="'projects[' + index + '][custom_category]'"
                                                x-model="project.customCategory"
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Your Role') }} *</label>
                                        <div x-data="searchableSelectProjectRole(project.id)" class="relative">
                                            <input type="hidden" :name="'projects[' + index + '][role]'" :value="selectedValue" x-model="selectedValue">
                                            <div class="relative">
                                                <input
                                                    type="text"
                                                    x-model="searchQuery"
                                                    ...
                                                    @keydown.arrow-up.prevent="highlightPrevious()"
                                                    @keydown.enter.prevent="selectHighlighted()"
                                                    :placeholder="selectedValue || '{{ __('Select your role') }}'"
                                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all pr-10"
                                                    autocomplete="off"
                                                >
                                                <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                        ...
                                        <input
                                            x-show="project.role === 'Other'"
                                            type="text"
                                            :name="'projects[' + index + '][custom_role]'"
                                            x-model="project.customRole"
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            rows="3"
                                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                                            placeholder="{{ __('Describe the project, your contributions, and outcomes...') }}"
                                        ></textarea>
                                        <p class="text-xs text-gray-500 mt-1"><span x-text="project.description ? project.description.length : 0"></span>/500 {{ __('characters') }}</p>
                                    </div>
                                </div>
                            </div>
                        </template>
                        ...
                        <button
                            ...
                            :class="{'opacity-50 cursor-not-allowed': formData.projects.length >= 5}"
                        >
                            <span x-text="formData.projects.length >= 5 ? '✓ Maximum 5 Projects Reached' : '+ Add Project (' + formData.projects.length + '/5)'"></span>
                        </button>
                    </div>
                </div>
            </div>
