...
>
    ...
    <div class="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6">
        <div class="text-center text-white">
            <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-3 sm:mb-4">{{ __('Discover Unique Products') }}</h1>
            <p class="text-sm sm:text-lg md:text-xl text-white/90 max-w-2xl mx-auto">{{ __('Explore handcrafted and unique products from talented creators and manufacturers') }}</p>
        </div>
    </div>
</section>
...
<section class="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
    <div class="max-w-[1440px] mx-auto px-4 sm:px-6 py-4">
        <form method="GET" action="{{ route('products', ['locale' => app()->getLocale()]) }}" class="flex flex-col md:flex-row gap-4">
            {{-- Search --}}
            <div class="flex-1">
                <div class="relative">
                    <input
